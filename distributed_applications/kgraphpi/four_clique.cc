...
#define MAX_COUNTING_PATTERN_SIZE 4
                        ...
                        case 3:
                            {
                                ...
                                //VertexId cached_ptx_2[std::min(nbrs.get_num_vertices(), prefix_1.get_num_vertices())];

                                VertexId buff_size = std::min(nbrs.get_num_vertices(), prefix_1.get_num_vertices()) * sizeof(VertexId);
                                VertexId * cached_ptx_2 = (VertexId*) engine->alloc_thread_local_scratchpad_buffer(context, buff_size);

                                VertexSet prefix_2(cached_ptx_2, 0);
                                prefix_1.intersect_with(&nbrs, &prefix_2);
                                // Scattering new extendable embedings or processing them (in the case that they are already complete)
                                VertexId num_last_v = prefix_2.get_num_vertices();
                                VertexId * last_v_list = prefix_2.get_vertices_list();
                                VertexId lower_bound = v_0;
                                lower_bound = std::max(lower_bound, v_2);
