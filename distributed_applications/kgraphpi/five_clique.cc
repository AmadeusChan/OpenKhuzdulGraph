                        ...
                        case 2:
                            {
                                ...
                                *(cached_ptx_1 - 1) = prefix_1.get_num_vertices();
                                ...
                                for (VertexId v_idx = 0; v_idx < num_v; ++ v_idx) {
                                    ...
                                    bool is_symmetry_broken = v <= v_1;
                                    if (is_symmetry_broken) continue;
                                    bool valid = v != v_0 && v != v_1;
                                    if (! valid) continue;
                                    // Scattering new extendable embeddings
                                    VertexId cached_obj_size = 0;
                        ...
                        case 3:
                            {
                                ...
                                *(cached_ptx_2 - 1) = prefix_2.get_num_vertices();
                                ...
                                for (VertexId v_idx = 0; v_idx < num_v; ++ v_idx) {
                                    ...
                                    bool is_symmetry_broken = v <= v_0 || v <= v_2;
                                    if (is_symmetry_broken) continue;
                                    bool valid = v != v_0 && v != v_1 && v != v_2;
                                    if (! valid) continue;
                                    // Scattering new extendable embeddings
                                    VertexId cached_obj_size = 0;
                                    engine->scatter(v, true, cached_obj_size, e, context);
                        ...
                        case 4:
                            {
                                ...
                                //VertexId cached_ptx_3[std::min(nbrs.get_num_vertices(), prefix_2.get_num_vertices())];

                                VertexId buff_size = std::min(nbrs.get_num_vertices(), prefix_2.get_num_vertices()) * sizeof(VertexId);
                                VertexId * cached_ptx_3 = (VertexId*) engine->alloc_thread_local_scratchpad_buffer(context, buff_size);

                                VertexSet prefix_3(cached_ptx_3, 0);
                                prefix_2.intersect_with(&nbrs, &prefix_3);
                                // Scattering new extendable embedings or processing them (in the case that they are already complete)
                                VertexId num_last_v = prefix_3.get_num_vertices();
                                VertexId * last_v_list = prefix_3.get_vertices_list();
                                VertexId lower_bound = v_1;
                                lower_bound = std::max(lower_bound, v_3);
                                ...
                                local_counter -= std::binary_search(last_v_list + trimed_num_last_v, last_v_list + num_last_v, v_2);
                                local_counter -= std::binary_search(last_v_list + trimed_num_last_v, last_v_list + num_last_v, v_3);
