...
void convert_graph(std::string input_graph_path, std::string output_graph_path) {
    ...
    srand(17);
    VertexId * shuffled_vertices = new VertexId [num_vertices];
    printf("Number of vertices: %u\n", num_vertices);
    for (VertexId v_i = 0; v_i < num_vertices; ++ v_i) {
        shuffled_vertices[v_i] = v_i;
    }
    for (VertexId v_i = 1; v_i < num_vertices; ++ v_i) {
        VertexId v_j = rand() % (v_i + 1);
        std::swap(shuffled_vertices[v_i], shuffled_vertices[v_j]);
    }
    ...
    for (VertexId v_i = 0; v_i < num_vertices; ++ v_i) {
        ...
        for (VertexId v_j_idx = 0; v_j_idx < num_neighbours; ++ v_j_idx) {
            ...
            edge_buffer[num_buffered_edges].src = shuffled_vertices[v_i];
            edge_buffer[num_buffered_edges].dst = shuffled_vertices[v_j];
            ++ num_buffered_edges;
...
int main(int argc, char ** argv) {
    if (argc != 3) {
        Debug::get_instance()->print("usage: to_shuffled_biedgelist_converter [Dwarves format dataset (*.dgraph)] [binary edge list format dataset]");
        exit(-1);
    }
